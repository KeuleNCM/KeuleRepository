/*
 * Copyright 2006 by Sean Luke and George Mason University Licensed under the
 * Academic Free License version 3.0 See the file "LICENSE" for more information
 */
...
/**
 * Display2D holds, displays, and manipulates 2D Portrayal objects, allowing the
 * user to scale them, scroll them, change how often they're updated, take
 * snapshots, and generate Quicktime movies. Display2D is Steppable, and each
 * time it is stepped it redraws itself. Display2D also handles double-click
 * events, routing them to the underlying portrayals as inspector requests.
 * 
 * <p>
 * In addition to various GUI widgets, Display2D holds a JScrollView which in
 * turn holds a Display2D.InnerDisplay2D (a JComponent responsible for doing the
 * actual drawing). Display2D can be placed in a JFrame; indeed it provides a
 * convenience function to sprout its own JFrame with the method createFrame().
 * You can put Display2D in your own JFrame if you like, but you should try to
 * call Display2D.quit() when the frame is disposed.
 * 
 * <p>
 * Display2D's constructor takes a height and a width; this will be the
 * "expected" height and width of the underlying portrayal region when the
 * Display2D is scaled to 1.0 (the default). The portrayals will also have an
 * origin at (0,0) -- the top left corner. Display2D will automatically clip the
 * portrayals to the area (0,0) to (width * scale, height * scale).
 * 
 * <p>
 * Display2D's step() method is typically called from the underlying schedule
 * thread; this means that it has to be careful about painting as Swing widgets
 * expect to be painted in the event loop thread. Display2D handles this in two
 * ways. First, on MacOS X, the step() method calls repaint(), which will in
 * turn call paintComponent() from the event loop thread at a time when the
 * underlying schedule thread is doing nothing -- see Console. Second, on
 * Windows and XWindows, the step() method immediately calls paintComponent().
 * Different OSes do it differently because MacOS X is far more efficient using
 * standard repaint() calls, which get routed through Quartz. The step() method
 * also updates various widgets using SwingUtilities.invokeLater().
 */
...
public class Display2D extends JComponent implements Steppable, Manipulating2D
{
    ...
    /**
     * If you have more than one Display2D in your simulation and you want them
     * to have different preferences, set each to a different key value. The
     * default value is DEFAULT_PREFERENCES_KEY. You may not have a key which
     * ends in a forward slash (/) when trimmed Key may be set to null (the
     * default).
     */
	...
	OptionPane(String title)
	{
	    ...
	    useNoBuffer
	    .setToolTipText("<html>When not using transparency on Windows/XWindows,<br>this method is often (but not always) faster</html>");
		...
		public void actionPerformed(ActionEvent e)
		{
		    ...
		    // if we're setting the system preferences, remove the local
	...
	void savePreferences(Preferences prefs)
	{
	    try
	    {
		prefs.putInt(DRAW_GRIDS_KEY, useNoBuffer.isSelected() ? 0
		...
		if (!Prefs.save(prefs))
		    Utilities.inform("Preferences Cannot be Saved",
		    "Your Java system can't save preferences.  Perhaps this is an applet?", this);
	    }
	...
	/**
	 * Resets the Option Pane Preferences by loading from the preference
	 * database
	 */
	void resetToPreferences()
	{
		...
		int val = appPrefs.getInt(
		DRAW_GRIDS_KEY,
		systemPrefs.getInt(DRAW_GRIDS_KEY,
		useNoBuffer.isSelected() ? 0 : useBuffer.isSelected() ? 1 : 2));
		if (val == 0)
		...
		// trigger resets by calling the listener. Don't bother with an
		// event
    ...
    /**
     * Removes all mouse listeners, mouse motion listeners, and Key listeners
     * from this component. Mostly used for kiosk mode stuff -- see the Howto
     */
    ...
    /**
     * The object which actually does all the drawing. Perhaps we should move
     * this out.
     */
	...
	/**
	 * Paints a movie, by drawing to a buffer, then encoding the buffer to
	 * disk, then optionally writing the buffer to the provided Graphics2D.
	 * If the Graphics2D is null, it's just written out to disk. This method
	 * will only write to disk when "appropriate", that is, if the current
	 * schedule has advanced to the point that a new frame is supposed to be
	 * outputted (given the frame rate of the movie). In any rate, it'll
	 * write to the Graphic2D if provided.
	 */
	public void paintToMovie(Graphics g)
	{
	    // although presently paintToMovie is called solely from
	    // paintComponent,
	    // which already has synchronized on the schedule, we do this anyway
	    // for
	    // good measure. See stopMovie() and startMovie() for why
	    // synchronization
	    // is important.
		...
		if (steps > lastEncodedSteps && shouldUpdate()
	...
	/**
	 * The default method for setting up the given hints. By default they
	 * suggest that Java2D emphasize efficiency over prettiness.
	 */
	public void setupHints(boolean antialias, boolean niceAlphaInterpolation,
	boolean niceInterpolation)
	{
	    unbufferedHints = new RenderingHints(RenderingHints.KEY_RENDERING,
	    RenderingHints.VALUE_RENDER_SPEED); // in general
	    ...
	    // -- if it's on, then circles can get drawn as squares (see woims
	    // demo at 1.5 scale)
	    // -- but if it's off, then stuff gets antialiased in pictures but
	    // not on a screenshot.
	    ...
	    niceAlphaInterpolation ? RenderingHints.VALUE_ALPHA_INTERPOLATION_QUALITY
	    : RenderingHints.VALUE_ALPHA_INTERPOLATION_SPEED);

	    bufferedHints = new RenderingHints(RenderingHints.KEY_RENDERING,
	    RenderingHints.VALUE_RENDER_SPEED); // in general
	...
	// could be called from the model thread OR the swing thread -- must be
	// careful
	...
	{
	    String s = "<html><font face=\"" + getFont().getFamily() + "\" size=\"-1\">";
		...
		for (int i = 0; i < hitObjects[x].numObjs; i++)
		{
		    ...
		    count++;
		    String status = p.portrayal
		    .getStatus((LocationWrapper) (hitObjects[x].objs[i]));
		    if (status != null)
			s += status; // might return null, sort of meaning
				     // "leave me alone"
		}
	    }
	    if (count == 0)
	...
	/**
	 * Swing's equivalent of paint(Graphics g). Called by repaint(). In turn
	 * calls paintComponent(g,false); You should not call this method
	 * directly. Instead you probably want to call paintComponent(Graphics,
	 * buffer).
	 */
	public synchronized void paintComponent(final Graphics g)
	{
	    // Swing gets overreaching when scaling in. This allows us to
	    // temporarily refuse to paint so we don't do so multiple times.
	    if (paintLock)
		return;
	...
	/**
	 * The top-level repainting method. If we're writing to a movie, we do a
	 * paintToMovie (which also does a buffered paint to the screen) else we
	 * do an ordinary paint. <tt>buffer</tt> determines if we do our
	 * ordinary paints buffered or not.
	 * 
	 * @deprecated use paintComponent() or paint(...)
	 */
	public void paintComponent(Graphics g, boolean buffer)
	{
	    synchronized (Display2D.this.simulation.state.schedule) // for
								    // getTime()
	    {
	...
	{
	    // This line is in case we're drawing from something other than the
	    // main
	    // event loop. In this situation, we need to get the view rect and
	    // figure
	    // our clip region accordingly. But port.getViewRect() is a problem
	    // because it's
	    // calling magic synchronized code inside it, and so can deadlock if
	    // Swing is
	    // locked (which it likely is if it's ALSO calling paintComponent
	    // and we beat
	    // it to it). So we maintain the last rect here using our own magic
	    // getViewRect
		...
		clip = clip.createIntersection(new Rectangle2D.Double(origindx, origindy, s.width,
		s.height));
	    }
	    return clip;
	}
	...
	/**
	 * Paints an image to the screen either buffered or unbuffered. If
	 * buffered, the buffer is returned for your convenience. If SHARED is
	 * true, then the buffer is shared internally, so if you want to hold
	 * onto it without it changing, you'll need to make a copy. If SHARED is
	 * false, then the buffer is given to you to do as you like and a new
	 * buffer is created next time (less efficient). This method is called
	 * from Swing when the window is resized or scrolled or whatnot, but
	 * it's also called as a result of the underlying model thread advancing
	 * a step so we can see what happened. In the second case, the method
	 * could conceivably be called both from the Swing event thread (if
	 * making a movie, or if in MacOS X) or from the underlying model thread
	 * (if in Windows or X Windows -- to change this, see Display2D.step).
	 * This is important because this if we're being called from the Swing
	 * event thread (repaint/movie/MacOS X), we must ensure that we're not
	 * updating the model at the same time the model is changing, and to do
	 * that we lock on the simulation's schedule so we know the model is
	 * done with step() methods and nothing's moving. But because we're
	 * blocking on the schedule, a deadlock opportunity arises. For example,
	 * what if some foolish person writes a step() method in the model which
	 * calls some blocking item in the Swing event thread? Then they're
	 * blocking waiting for the Swing event thread to come available again,
	 * but the event thread is blocking because we're waiting for them to
	 * get out of the model. A similar problem could arise if in
	 * implementing a portrayal's draw method the user decides to call some
	 * blocking item in Swing, because this paint() method could conceivably
	 * be called from the model itself (in Windows/XWindows), and it's
	 * possible that at the very same time the user has pressed the stop
	 * button or done some operation in Swing which is blocking waiting for
	 * the schedule. Long story short, try not to call a blocking method on
	 * Swing from inside this method.
	 */
	public BufferedImage paint(final Graphics graphics, boolean buffered, boolean shared)
	{
		...
		if (!shared)
		    buffer = null; // kill it so paintBuffered(graphics,clip)
	...
	/**
	 * Draws the image into a buffer, then IF graphics is not null, draws
	 * the resulting buffer to the graphics. Returns the buffer, which is
	 * shared internally, so you need to copy out of it as soon as possible.
	 */
	BufferedImage paintBuffered(final Graphics2D graphics, final Rectangle2D clip)
	{
	    ...
	    g.dispose(); // because we got it with getGraphics(), we're
			 // responsible for it
	...
	/**
	 * Paints an image unbuffered inside the provided clip. Not
	 * synchronized. You should probably call paintComponent() instead.
	 */
	void paintUnbuffered(Graphics2D g, Rectangle2D clip)
	{
		...
		{
		    g.setPaint(backdrop);
		    g.fillRect((int) clip.getX(), (int) clip.getY(), (int) clip.getWidth(),
		    (int) clip.getHeight());
		}
		...
		while (iter.hasNext())
		{
			...
			// MacOS X 10.3 Panther has a bug which resets the clip,
			// YUCK
			g.setClip(g.getClip());

			// do the drawing
			p.portrayal.draw(p.portrayal.getField(), // I could have
								 // passed null
								 // in here too
	 ...
	 * clipping rect, because getViewRect calls some unknown synchronized
	 * gunk further up in Swing; thus if I'm in Windoze and splat to the
	 * screen from my own thread, and at the same time the Swing thread is
	 * trying to draw me, we have a problem -- it grabs the unknown gunk
	 * lock, I synchronize on this, then I try to call getViewRect on the
	 * port and lock, and we're both hung. if we do redrawing via repaint()
	 * like in MacOS X, then we're fine, but if we do it a-la X or Windows,
	 * this bug rears its ugly head. So we get the most recent viewRect and
	 * set it here and keep it for when the redraw needs it, so it doesn't
	 * have to ask for it from Swing (which could be locked).
	 */

	Rectangle    viewRect     = new Rectangle(0, 0, 0, 0); // no access
							       // except via
							       // synchronization

	/**
	 * Lock for the viewRect above. Don't want to lock on the Display2D
	 * itself.
	 */
    ...
    {
	/**
	 * The translation and scale of the FieldPortrayal. Presently this is
	 * always 0,0 translation and 1.0 scale, but we'll allow the user to
	 * change this soon.
	 */
	...
	/**
	 * The name of the portrayal, as shown in the Layers menu on the
	 * Display2D window
	 */
	String	     name;
	...
	/**
	 * Creates a menu item which selects or unselects the portrayal for
	 * drawing.
	 */
	FieldPortrayal2DHolder(FieldPortrayal2D p, String n, Rectangle2D.Double bounds,
	boolean visible)
	{
	    this.bounds = bounds;
	    portrayal = p;
    ...
    // Windows draws faster by handling our own double buffering into our own
    // Java-created
    // buffered image. MacOS X draws faster by doing a repaint and letting the
    // OS handle
    // the buffering. Dunno about X Windows, haven't checked, I presume it's
    // same situation
    ...
    /*
     * Sets various MacOS X features. This text is repeated in Console.java,
     * Display2D.java, and Display3D.java The reason for the repeat is that the
     * UseQuartz property must be set a precise time -- for example, we can't
     * just use this static to call a common static method -- it doesn't work
     * :-( Otherwise we'd have made one static method which did all this stuff,
     * duh.
     */
    static
    {
	// use heavyweight tooltips -- otherwise they get obscured by the
	// Canvas3D
	...
	try
	{
	    ...
	    System.setProperty("Quaqua.TabbedPane.design", "auto"); // UI
								    // Manager
								    // Properties
								    // docs
								    // differ
	    ...
	    .forName("ch.randelshofer.quaqua.QuaquaManager").getMethod("getLookAndFeelClassName",
	...
	try
	// now we try to set certain properties if the security permits it
	{
	    // turn on hardware acceleration on MacOS X. As of September 2003,
	    // 1.3.1
	    // turns this off by default, which makes 1.3.1 half the speed (and
	    // draws
	    // objects wrong to boot).
	    System.setProperty("com.apple.hwaccel", "true"); // probably
							     // settable as an
							     // applet. D'oh!
							     // Looks like it's
							     // ignored.
	    System.setProperty("apple.awt.graphics.UseQuartz", "true"); // counter
									// the
									// awful
									// effect
									// in OS
	    ...
	    // 1.3.1 version -- Apple gives us an obnoxious warning in 1.4.1
	    // when
	    // we call forth an open/save panel saying we should now use
	    // apple.awt.use-file-dialog-packages instead, as if 1.3.1 isn't
	    // also
	    // in common use...
    ...
    /**
     * Returns icons for a given filename, such as "Layers.png". A utility
     * function.
     */
    ...
    public static final Object[]  REDRAW_OPTIONS   = new Object[]
						   { "Steps/Redraw", "Model Secs/Redraw",
	    "Real Secs/Redraw", "Always Redraw", "Never Redraw" };
     ...
     */
    long			  lastEncodedSteps = -1;			    // because
										     // we
										     // want
										     // to
										     // encode
    ...
    /**
     * The 2D display inside the scroll view. Does the actual drawing of the
     * simulation.
     */
    ...
    /**
     * The list of portrayals the insideDisplay draws. Each element in this list
     * is a Portrayal2DHolder.
     */
    ...
    JViewport		     port;
    /**
     * The stoppable for the repeat object which redraws the Display2D in the
     * schedule.
     */
    Stoppable		     stopper;
    ...
    public JToggleButton	  layersbutton;				     // for
										     // popup
    /** The refresh menu */
    public JPopupMenu	     refreshPopup;
    /** The button which pops up the refresh menu */
    public JToggleButton	  refreshbutton;				    // for
										     // popup
    ...
    double			scale	    = 1.0;
    final Object		  scaleLock	= new Object();		  // scale
										     // lock
    ...
    // public void setInterval(long i) { synchronized(intervalLock) { if (i > 0)
    // interval = i; } }
    ...
    /**
     * Returns true if the Display2D is clipping the drawing area to the
    ...
    /**
     * Sets the Display2D to clip or to not clip to the user-specified height
     * and width when drawing
     */
    ...
    /**
     * Backdrop color or other paint. This is the color/paint that the
     * simulation is whitewashed with prior to the portrayals redrawing
     * themselves. This differs from the scroll view's BACKGROUND color, which
     * is the color of any area that the simulation doesn't draw on.
     */
    Paint backdrop = Color.white; // default. It'll get changed.

    /**
     * Specify the backdrop color or other paint. The backdrop is the region
     * behind where the simulation actually draws. If set to null, no
     * color/paint is used -- and indeed the background you're drawing on is not
     * defined. Only set to null if you know you're filling the entire
     * background with something else anyway.
     */
    ...
    /**
     * Returns the backdrop color or paint. The backdrop is the region behind
     * where the simulation actually draws. If set to null, no color/paint is
     * used.
     */
    ...
    /**
     * Quits the Display2D. Okay, so finalize is evil and we're not supposed to
     * rely on it. We're not. But it's an additional cargo-cult programming
    ...
    /**
     * Quits the Display2D. Called by the Display2D's frame if the Display2D
     * made the frame itself. Also called by finalize(). Otherwise you should
     * call this method before destroying the Display2D.
     */
    public void quit()
    {
	if (stopper != null)
    ...
    /**
     * Resets the Display2D so it reschedules itself and clears out all
     * selections. This is useful when reusing the Display2D.
     */
    public void reset()
    {
     ...
     * default width and height. Portrayals are drawn on-screen in the order
     * that they are attached; thus the "top-most" portrayal will be the last
     * one attached.
     */
    ...
    /**
     * Attaches a portrayal to the Display2D, along with the provided
     * human-readable name for the portrayal. The portrayal's attached origin,
     * width and height is given in the bounds rectangle. Portrayals are drawn
     * on-screen in the order that they are attached; thus the "top-most"
     * portrayal will be the last one attached.
     */
     ...
     * human-readable name for the portrayal. The portrayal will be attached
     * with an origin at (0,0) and a width and height equal to the Display2D's
     * default width and height. The portrayal may be set to initially visible
     * or not visible. Portrayals are drawn on-screen in the order that they are
     * attached; thus the "top-most" portrayal will be the last one attached.
     */
    ...
    /**
     * Attaches a portrayal to the Display2D, along with the provided
     * human-readable name for the portrayal. The portrayal's attached origin is
     * given with the coordinates provided. The width and height will be equal
     * to the Display2D's default width and height
     * (display2D.insideDisplay.width and display2D.insideDisplay.height
     * respectively). To put the origin at the center of the display, you can
     * set the x and y coordinates to display2D.insideDisplay.width/2,
     * display2D.insideDisplay.height/2). The portrayal may be set to initially
     * visible or not visible. Portrayals are drawn on-screen in the order that
     * they are attached; thus the "top-most" portrayal will be the last one
     * attached.
     */
    public void attach(FieldPortrayal2D portrayal, String name, double x, double y, boolean visible)
    {
	attach(portrayal, name, new Rectangle2D.Double(x, y, insideDisplay.width,
	insideDisplay.height), visible);
    }

    /**
     * Attaches a portrayal to the Display2D, along with the provided
     * human-readable name for the portrayal. The portrayal's attached origin,
     * width and height is given in the bounds rectangle. The portrayal may be
     * set to initially visible or not visible. Portrayals are drawn on-screen
     * in the order that they are attached; thus the "top-most" portrayal will
     * be the last one attached.
     */
    public void attach(FieldPortrayal2D portrayal, String name, Rectangle2D.Double bounds,
    boolean visible)
    {
    ...
    /**
     * A convenience function: creates a popup menu item of the given name
     * which, when selected, will display the given inspector in the Console.
     * Used rarely, typically for per-field Inspectors.
     */
    ...
    /**
     * Creates a Display2D with the provided width and height for its portrayal
     * region, attached to the provided simulation. The interval is ignored.
     * 
     * @deprecated
     */
    ...
    /**
     * Creates a Display2D with the provided width and height for its portrayal
     * region, attached to the provided simulation, and displaying itself with
     * the given interval (which must be > 0).
     */
	...
	final Color transparentBackground = new JPanel().getBackground(); // sacrificial
									  // JPanel

	// create the inner display and put it in a Scroll Panel
	insideDisplay = new InnerDisplay2D(width, height);
	display = new JScrollPane(insideDisplay, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
	...
	insideDisplay.setOpaque(true); // radically increases speed in OS X,
				       // maybe others
	// Bug in Panther causes this color to be wrong, ARGH
	// port.setBackground(UIManager.getColor("window")); // make the nice
	// stripes on MacOS X
	insideDisplay.setBackground(UIManager.getColor("Panel.background"));
	display.setBackground(UIManager.getColor("Panel.background")); // this
								       // is the
								       // one
								       // that
								       // has
	...
	{
	    public Dimension getPreferredSize() // we want to be as compressible
						// as necessary
	    {
	...
	layersbutton.addMouseListener(new MouseAdapter()
	{
	    ...
	    {
		popup.show(e.getComponent(), 0, // layersbutton.getLocation().x,
	...
	refreshbutton.addMouseListener(new MouseAdapter()
	{
	    ...
	    {
		rebuildRefreshPopup();
		refreshPopup.show(e.getComponent(), 0,
	...
	/*
	 * insideDisplay.addMouseWheelListener(new MouseWheelListener() { public
	 * void mouseWheelMoved(MouseWheelEvent e) { if (handleMouseEvent(e)) {
	 * repaint(); return; } } });
	 */
	...
	scaleField = new NumberTextField("  Scale: ", 1.0, true)
	{
		...
		// lock the paint lock so we don't try repainting until we
		// request it.
		...
		// scroll to keep the zoomed-in region centered -- this is
		// prettier
		...
		Point topleft = new Point((int) (centerx - r.width / 2.0),
		(int) (centery - r.height / 2.0));
		if (topleft.x < 0)
		    topleft.x = 0;
		if (topleft.y < 0)
		    topleft.y = 0;
    ...
    /**
     * Returns LocationWrappers for all the objects which fall within the
     * coordinate rectangle specified by rect. This rectangle is in the
     * coordinate system of the (InnerDisplay2D) component inside the scroll
     * view of the Display2D class. The return value is an array of Bags. For
     * each FieldPortrayal attached to the Display2D, one Bag is returned
     * holding all the LocationWrappers for objects falling within the rectangle
     * which are associated with that FieldPortrayal's portrayed field. The
     * order of the Bags in the array is the same as the order of the
     * FieldPortrayals in the Display2D's <code>portrayals</code> list.
     */
    ...
    /**
     * Returns LocationWrappers for all the objects which overlap with the point
     * specified by 'point'. This point is in the coordinate system of the
     * (InnerDisplay2D) component inside the scroll view of the Display2D class.
     * The return value is an array of Bags. For each FieldPortrayal attached to
     * the Display2D, one Bag is returned holding all the LocationWrappers for
     * objects overlapping with the point which are associated with that
     * FieldPortrayal's portrayed field. The order of the Bags in the array is
     * the same as the order of the FieldPortrayals in the Display2D's
    ...
    /**
     * Constructs a DrawInfo2D for the given portrayal, or null if failed. O(num
     * portrayals). Uses the given point as a clip.
     */
    public DrawInfo2D getDrawInfo2D(FieldPortrayal2D portrayal, Point2D point)
    {
	return getDrawInfo2D(portrayal,
	new java.awt.geom.Rectangle2D.Double(point.getX(), point.getY(), 1, 1));
    }

    /**
     * Constructs a DrawInfo2D for the given portrayal, or null if failed. O(num
     * portrayals). Uses the given clip.
     */
	...
	// we floor to an integer because we're dealing with exact pixels at
	// this point
	(int) (holder.bounds.x * scale) + origindx, (int) (holder.bounds.y * scale) + origindy,
	(int) (holder.bounds.width * scale), (int) (holder.bounds.height * scale));
    ...
    /**
     * Determines the inspectors appropriate for the given selection region
     * (rect), and sends them on to the Controller.
     */
    ...
    /**
     * Creates a frame holding the Display2D. This is the best method to create
     * the frame, rather than making a frame and putting the Display2D in it. If
     * you prefer the latter, then you need to handle two things. First, when
     * the frame is disposed, you need to call quit() on the Display2D. Second,
     * if you care about distribution to MacOS X Java 1.3.1, you need to call
     * Utilities.doEnsuredRepaint(header) whenever the window is resized.
     */
	...
	frame.addComponentListener(new ComponentAdapter()
	{
		...
		display.getHorizontalScrollBar().setUnitIncrement(
		display.getViewport().getWidth() / SCROLL_BAR_SCROLL_RATIO);
		display.getVerticalScrollBar().setUnitIncrement(
		display.getViewport().getHeight() / SCROLL_BAR_SCROLL_RATIO);
	    }
	});
    ...
    /**
     * Returns the frame holding this Component. If there is NO such frame, an
     * error will be generated (probably a ClassCastException).
     */
    ...
    //
    // // Why are we using PNG? For a couple of reasons. First, GIF only
    // supports 256 colors. That had begun
    // // to bite us. Second, JPEG is good for photos but quite poor for
    // screenshots which have lots of
    // // straight lines and aliased stuff. PNG is the RIGHT choice for what we
    // need to do. Unfortunately
    // // it's not properly supported by old versions Internet Exploder, and so
    // to make web-ready snapshots
    // // you may need to convert it. :-(
    //
    // // We're using our own small PNG package (see sim.util.media) because the
    // JAI isn't standard across
    // // all platforms (notably 1.3.1) and at the time we made the call, it
    // wasn't available on the Mac at all.
    //
    ...
    // PNGEncoder tmpEncoder = new PNGEncoder(img,
    // false,PNGEncoder.FILTER_NONE,9);
    ...
    // // snap the shot FIRST
    // Graphics g = insideDisplay.getGraphics();
    // BufferedImage img = insideDisplay.paint(g,true,false); // notice we're
    // painting to a non-shared buffer
    // try
    // {
    // sacrificialObj = Class.forName("com.lowagie.text.Cell").newInstance(); //
    // sacrificial
    ...
    // }
    //
    // g.dispose(); // because we got it with getGraphics(), we're responsible
    // for it
    //
    ...
    // if (havePDF)
    // {
    // Object[] options = { "Cancel", "Save to PNG Bitmap", "Save to PDF",
    // "Save to PDF with no Backdrop" };
    // result = JOptionPane.showOptionDialog(getFrame(),
    // "Save window snapshot to what kind of file format?", "Save Format",
    ...
    // setBackdrop(null);
    // PDFEncoder.generatePDF(port, new File(fd.getDirectory(),
    // Utilities.ensureFileEndsWith(fd.getFile(),".pdf")));
    // precise = oldprecise;
    ...
    /**
     * Starts a Quicktime movie on the given Display2D. The size of the movie
     * frame will be the size of the display at the time this method is called.
     * This method ought to be called from the main event loop. Most of the
     * default movie formats provided will result in a gigantic movie, which you
     * can re-encode using something smarter (like the Animation or Sorenson
     * codecs) to put to a reasonable size. On the Mac, Quicktime Pro will do
	...
	// than the Swing thread (see step(...) below). This allows us to
	// guarantee,
	// everywhere where movieMaker is set (to null or to new), that
	// paintToMovie
	...
	synchronized (Display2D.this.simulation.state.schedule)
	{
		...
		"MASON Applet Restriction", JOptionPane.OK_OPTION, JOptionPane.ERROR_MESSAGE, null,
		options, options[0]);
		return;
	    }
		...
		if (console.getPlayState() == Console.PS_STOPPED) // either
								  // after
								  // simulation
								  // or we just
								  // started the
		...
		// capture the currently shown frame (important if we just
		// paused the simulation)
		insideDisplay.paintToMovie(null);

		// set ourselves up to quit when stopped
		simulation.scheduleAtEnd(new Steppable() // to stop movie when
							 // simulation is
							 // stopped
		{
    ...
    /**
     * Stops a Quicktime movie and cleans up, flushing the remaining frames out
     * to disk. This method ought to be called from the main event loop.
     */
	...
	synchronized (Display2D.this.simulation.state.schedule)
	{
		...
		"JMF Movie Generation Bug", JOptionPane.OK_OPTION, JOptionPane.WARNING_MESSAGE,
		null, options, options[0]);
	    }
	    movieMaker = null;
	...
	else if (updateRule == UPDATE_RULE_STEPS)
	{
	    ...
	    val = (lastStep < 0 || stepInterval == 0 || step - lastStep >= stepInterval || // clearly
											   // need
											   // to
											   // update
	    lastStep % stepInterval >= step % stepInterval); // on opposite
							     // sides of a tick
	    if (val)
		lastStep = step;
	}
	else if (updateRule == UPDATE_RULE_WALLCLOCK_TIME)
	{
	    ...
	    val = (lastWall == 0 || wallInterval == 0 || wall - lastWall >= wallInterval || // clearly
											    // need
											    // to
											    // update
	    lastWall % wallInterval >= wall % wallInterval); // on opposite
							     // sides of a tick
	    if (val)
		lastWall = wall;
	}
	else if (updateRule == UPDATE_RULE_INTERNAL_TIME)
	{
	    ...
	    val = (lastTime == 0 || timeInterval == 0 || time - lastTime >= timeInterval || // clearly
											    // need
											    // to
											    // update
	    lastTime % timeInterval >= time % timeInterval); // on opposite
							     // sides of a tick
	    if (val)
		lastTime = time;
	}
	...
	for (int x = 0; x < selectedWrappers.size(); x++)
	{
	    ...
	    if (portrayal.handleMouseEvent(simulation, this, wrapper, event, getDrawInfo2D(f, p),
	    SimplePortrayal2D.TYPE_SELECTED_OBJECT))
	    {
	...
	// We go backwards through the bag list so top elements are selected
	// first
	...
	for (int x = hitObjects.length - 1; x >= 0; x--)
		...
		Object obj = wrapper.getObject();
		SimplePortrayal2D portrayal = (SimplePortrayal2D) (f.getPortrayalForObject(obj));
		if (portrayal.handleMouseEvent(simulation, this, wrapper, event,
		getDrawInfo2D(f, p), SimplePortrayal2D.TYPE_HIT_OBJECT))
		{
		    simulation.controller.refresh();
		    return true;
		}
	...
	skipField = new NumberTextField(null, 1, false)
	{
	    ...
	    {
		double val;
		if (updateRule == UPDATE_RULE_ALWAYS || updateRule == UPDATE_RULE_NEVER) // shouldn't
											 // have
											 // happened
		{
		    val = 0;
		...
		// if (updateRule == UPDATE_RULE_INTERNAL_TIME)
		{
		    val = newValue;
		    if (newValue < 0)
			newValue = timeInterval;
		    timeInterval = val;
		}
	...
	skipListener.actionPerformed(null); // have it update the text field
					    // accordingly
    }
    ...
    /**
     * Steps the Display2D in the GUIState schedule. If we're in MacOS X, this
     * results in a repaint() request generated. If we're in Windows or X
     * Windows, this results in a direct call to paintComponent on the
     * insideDisplay.
     */
	...
	if (shouldUpdate()) // time to update!
	{
	    ...
	    && (getFrame().getExtendedState() & java.awt.Frame.ICONIFIED) == 0) // not
										// minimized
										// on
										// the
										// Mac
	    ...
	    else if (movieMaker != null) // we're not being displayed but we
					 // still need to output to a movie
	    {
		insideDisplay.paintToMovie(null);
	    }
